...
func DrawHLine(screen tcell.Screen, x, y, width int, style tcell.Style) {
	for i := 0; i < width; i++ {
		screen.SetContent(x+i, y, rune(symbols.HLine), nil, style)
	}
}
...
func DrawVLine(screen tcell.Screen, x, y, height int, style tcell.Style) {
	for i := 0; i < height; i++ {
		screen.SetContent(x, y+i, rune(symbols.VLine), nil, style)
	}
}
...
func DrawBox(screen tcell.Screen, x, y, width, height int, style tcell.Style) {
	// Draw corners
	screen.SetContent(x, y, rune(symbols.TLCorner), nil, style)
	screen.SetContent(x+width-1, y, rune(symbols.TRCorner), nil, style)
	screen.SetContent(x, y+height-1, rune(symbols.BLCorner), nil, style)
	screen.SetContent(x+width-1, y+height-1, rune(symbols.BRCorner), nil, style)

	// Draw horizontal lines
	for i := 1; i < width-1; i++ {
		screen.SetContent(x+i, y, rune(symbols.HLine), nil, style)
		screen.SetContent(x+i, y+height-1, rune(symbols.HLine), nil, style)
	}

	// Draw vertical lines
	for i := 1; i < height-1; i++ {
		screen.SetContent(x, y+i, rune(symbols.VLine), nil, style)
		screen.SetContent(x+width-1, y+i, rune(symbols.VLine), nil, style)
	}
}
